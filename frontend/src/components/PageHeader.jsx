...
export function PageHeader({ eyebrow, title, description, action, meta }) {
  return (
    <section className="rounded-xl border border-fuel-line bg-white p-4 shadow-soft sm:p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          {eyebrow && (
            <p className="text-xs font-black uppercase tracking-[0.16em] text-fuel-green">{eyebrow}</p>
          )}
          <h2 className="mt-1 truncate text-2xl font-extrabold leading-tight text-fuel-ink sm:text-3xl">{title}</h2>
          {description && <p className="mt-2 max-w-2xl text-sm font-medium text-slate-600">{description}</p>}
        </div>
        {(action || meta) && (
...
export const primaryButton =
  "inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-fuel-green px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-fuel-deep disabled:cursor-not-allowed disabled:opacity-60";
...
export function Pill({ children, tone = "green" }) {
  ...
  return (
    <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-bold ${tones[tone] || tones.green}`}>
      {children}
    </span>
  );
}
